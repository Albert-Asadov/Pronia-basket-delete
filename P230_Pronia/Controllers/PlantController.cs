...
namespace P230_Pronia.Controllers
{
    ...
    public class PlantController : Controller
    {
        ...
        public IActionResult RemoveItemInBasket(int id)
        {
            var cookes = HttpContext.Request.Cookies["basket"];
            var basket = JsonConvert.DeserializeObject<CookieBasketVM>(cookes);
            var item = basket.CookieBasketItemVMs.FirstOrDefault(b => b.Id == id);
            if (item is not null)
            {
